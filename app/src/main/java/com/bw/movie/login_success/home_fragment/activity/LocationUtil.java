...
public class LocationUtil implements AMapLocationListener {
    ...
    public MarkerOptions getMarkerOption(String str, double lat, double lgt){
        ...
        markerOptions.snippet("纬度:" + lat + "   经" +
                "度:" + lgt);
        markerOptions.period(100);
        EventBus.getDefault().postSticky(new MessageList("location",str));
        EventBus.getDefault().postSticky(new MessageList("jidu",lgt));
        EventBus.getDefault().postSticky(new MessageList("weidu",lat));
        Log.i("TAG",str+lat+lgt);
